...
module.exports = {
  ...
  deploy: {
    production: {
      ...
      'post-deploy': 'mkdir ~/mesto-backend && cp -Rf ./backend/* ~/mesto-backend && cp ./.env ~/mesto-backend && cp ./.env.production ~/mesto-backend && cd ~/mesto-backend && rm -rf ~/repo && npm i && npm run build && pm2 start ecosystem.config.js',
    },
  },
};
